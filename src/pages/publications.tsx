...
import {PUBLICATIONS, TOPICS, type PublicationType} from '@site/src/data/publications';
...
export default function PublicationsPage(): ReactNode {
    ...
    const topics = useMemo(
        () => [
            ALL,
            ...TOPICS.filter((candidate) => PUBLICATIONS.some((p) => p.topics.includes(candidate))),
        ],
        [],
    );
    ...
    const filtered = useMemo(
        () => (topic === ALL ? PUBLICATIONS : PUBLICATIONS.filter((p) => p.topics.includes(topic))),
        [topic],
    );
    ...
    return (
        ...
        >
            <PageHeader title="Papers & preprints">
